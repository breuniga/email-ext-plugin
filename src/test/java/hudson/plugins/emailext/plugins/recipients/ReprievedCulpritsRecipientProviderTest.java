...
public class ReprievedCulpritsRecipientProviderTest {
    ...
    public void testAddRecipients() throws Exception {
        ...
        TestUtilities.checkRecipients(build4, new ReprievedCulpritsRecipientProvider());
        
        // --
        final WorkflowRun build5 = PowerMockito.mock(WorkflowRun.class);
        PowerMockito.when(build5.getResult()).thenReturn(Result.SUCCESS);
        MockUtilities.addChangeSet(build5, "C");
        PowerMockito.when(build5.getPreviousCompletedBuild()).thenReturn(build4);
        
        TestUtilities.checkRecipients(build5, new ReprievedCulpritsRecipientProvider(), "A", "B");

        // --
        final WorkflowRun build6 = PowerMockito.mock(WorkflowRun.class);
        PowerMockito.when(build6.getResult()).thenReturn(Result.FAILURE);
        MockUtilities.addChangeSet(build6, "A");
        PowerMockito.when(build6.getPreviousCompletedBuild()).thenReturn(build5);
        TestUtilities.checkRecipients(build6, new ReprievedCulpritsRecipientProvider());

        final WorkflowRun build7 = PowerMockito.mock(WorkflowRun.class);
        PowerMockito.when(build7.getResult()).thenReturn(Result.SUCCESS);
        MockUtilities.addChangeSet(build7, "C");
        PowerMockito.when(build7.getPreviousCompletedBuild()).thenReturn(build6);
        TestUtilities.checkRecipients(build7, new ReprievedCulpritsRecipientProvider(), "A");
